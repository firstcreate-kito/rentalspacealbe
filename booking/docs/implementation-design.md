...
- [ ] Stripe 銀行振込／コンビニの**領収書を自動発行**にするか（推奨：自動。Stripe が入金を電子確認するため。§15.9）。
...
- [x] **決済手段＝カード／請求書払い／Stripe銀行振込／コンビニ払いを有効化**。自動返金はカードのみ、銀行振込・コンビニは顧客とやりとりして銀行振込返金（§15.7）。
...
| **請求書払い（自社口座振込）** | 入金確認後（手動） | 銀行振込（返金先口座を収集） | 手動 |
...
> 追加課金（値上がり差額）の自動実行も**カードのみ**。請求書払い/銀行振込/コンビニでの差額はカード同様の自動課金ができないため、承認後に管理者が請求書 or 追加入金案内で対応する（手動）。
...
- **領収書**：入金の証明。カードは自動発行・送付。**請求書払い（自社口座振込）は管理者が入金確認後に手動発行**（§2.28）。
- **支払方法別の発行タイミング**：
  | 支払方法 | 請求書 | 領収書 |
  |---|---|---|
  | カード | 確定（入金済）時に自動送付 | 自動発行・送付 |
  | 請求書払い（自社口座振込） | 予約確定時に自動送付（振込額が判る） | 管理者が入金確認後に手動発行 |
  | Stripe 銀行振込／コンビニ | 入金→確定時に自動送付 | Stripe が入金を電子確認するため**自動発行を推奨**（§13-F 要確認） |
