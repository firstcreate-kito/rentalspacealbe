...
- [ ] 申請時に予約枠を **保持**（承認で解放）でよいか。※暫定：保持。
- [ ] セルフ即時キャンセル/変更（優良/VIP）を **全件承認制に統一**するか、一部ランクは承認不要で即時を残すか。
...
- **完全自動化はしない。全ての変更・キャンセルは管理者承認を経る（承認ゲート方式）。** 返金・差額課金の**計算と実行（Stripe API）はシステム**が行い、**実行の引き金は管理者の承認**とする。管理者は Stripe 管理画面を操作しない。
...
- セルフ即時キャンセル/変更（優良/VIP）の扱いは §13-F 要確認（全件承認制に統一するか）。
