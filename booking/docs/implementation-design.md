...
- [x] マイページ変更リクエストのモーダルの**キャンセルポリシー／変更ポリシー表示＝モーダル内アコーディオン**に決定（モーダル重ね・同一タブ遷移は不可。正式ページを持つ場合は別タブ）。ポリシー本文は単一管理。§15.10。
...
- **表示方式＝モーダル内アコーディオン（決定）**。画面遷移させず、同じモーダル内でポリシーを展開／折りたたむ（文脈を保つ）。**「モーダルの上にモーダル」を重ねる実装は避ける**（特にスマホで操作性が悪化）。同じタブでのページ遷移は不可（文脈が切れる）。
- 正式なポリシーページを別途持ちたい場合（法務・SEO 用）は、そのページへのリンクを**別タブ（`target="_blank"`）で開く**ことでモーダルを壊さず両立可（任意）。
- **ポリシー本文は1か所で単一管理**し、アコーディオンと（あれば）ポリシーページで同一文言を出す（二重管理による食い違いを防ぐ）。格納先は `system_settings` 等の1レコード or ポリシー用テーブルで一元化。
