...
本文に赤い `要確認（仮）` の印が **5か所** 残っています（`style.css` の `.tbd`）。下の1〜3が確定すれば消えます。
確定したら印ごと文言を差し替えてください。

### お答えが必要なもの

| # | 項目 | なぜ必要か | 印の場所 |
|---|---|---|---|
| 1 | **法人登記・住所利用の可否** | 仮事務所を探す人がまず確認する項目。可否どちらでも明記が要る | スペック表・FAQ |
| 2 | **郵便物・宅配便の受け取り** | スタッフ常駐ではない（近隣の事務所に在席）ため運用の説明が要る | スペック表・FAQ |
| 3 | **連日・長期の料金** | 「日数に応じてお見積り」のままにするか、週単位・月単位の料金表を出すか | 料金表 |
| 4 | **長期利用時の土足の扱い** | 通常は土足不可（有料オプション5,500円）。数週間の事務所利用での運用。いまはFAQで「ご相談ください」としてある | FAQ（印なし） |
| 5 | **広告のコンバージョンラベル** | `HANDOVER.md` §6 のとおり `AW-17762210452` を流用中。LP別に分けるかどうか | — |
| 6 | **フォーム側のhiddenフィールド** | CTAは `?venue=meieki-free&plan=office` を付与済み。`CLAUDE.md` 4-5 のとおりフォーム側の受け取りが要対応 | — |

### 処理済み

| 項目 | 対応 |
|---|---|
| 大型什器・複合機の搬入条件 | エレベーターの実寸（開口部 W85×H210cm／かご内 W125×D135×H230cm）を本文に明記し、超える場合は事前相談とした |
| OGP画像 | `office-ogp.webp`（1200×630）を用意して差し替え |
| 検索への露出 | `noindex, follow`（上記の理由） |

### 反映後に実機で確認すること

| 項目 | 見かた |
|---|---|
| 地図の埋め込み | `maps?q=...&output=embed` の簡易形式を使用。表示されない場合は `maps/embed?pb=` 形式に差し替える |
| 計測 | GA4のリアルタイムで `to_form_click` / `tel_click` / `line_click` / `matterport_open` / `view_pricing` が飛ぶか |
| LINEボタン | PCでQRモーダル、スマホでLINEアプリが開くか（`CLAUDE.md` 4-5） |
